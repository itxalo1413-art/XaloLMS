  ...
  return (
    <AcaLayout>
      <AcaTopbar
        ...
        subtitle="Quản lý đồng bộ điểm số đầu vào và nhập điểm thi cuối khóa của học viên."
      />
                    ...
                    return (
                      <tr key={st.id} className="hover:bg-zinc-50/50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-zinc-500">
                          <select
                            value={st.classId || ""}
                            onChange={(e) => void handleAssignClass(st.id, e.target.value)}
                            className="h-8 rounded-lg border border-zinc-200 bg-white px-2 text-xs font-bold text-foreground outline-none focus:border-primary cursor-pointer shadow-2xs max-w-[160px]"
                          >
