...
const graderNavItems: NavItem[] = [
  {
    href: GRADER_PATHS.writing,
    label: "Chấm Writing",
    desc: "Chấm bài + tiêu chí BCB Writing",
  },
  {
    href: GRADER_PATHS.speaking,
    label: "Test Speaking",
    desc: "Chấm ca + tiêu chí BCB Speaking",
  },
  {
    href: GRADER_PATHS.freeSlots,
    label: "Đăng ký lịch rảnh",
    desc: "Daily schedule & lịch rảnh Grader",
  },
];
      ...
      <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl bg-white shadow-soft">
        ...
        <nav className="flex min-h-0 flex-1 flex-col overflow-hidden px-3 py-3">
          ...
          <div className="mt-2 shrink-0 space-y-0.5 border-t border-zinc-100 bg-white pt-3">
            ...
            <Link
              href={GRADER_PATHS.profile}
              className={[
                "group flex items-center gap-2.5 rounded-xl px-3 py-2 transition-all duration-200",
                pathname.startsWith(GRADER_PATHS.profile)
                  ? "bg-primary text-white shadow-premium"
                  : "text-muted hover:bg-primary-soft/70 hover:text-foreground",
              ].join(" ")}
            >
              ...
              >
                <svg
                  ...
                  fill="none"
                  stroke="currentColor"
                  strokeWidth={pathname.startsWith(GRADER_PATHS.profile) ? "2.5" : "2"}
                >
              ...
              <div className="min-w-0 flex-1">
                <div
                  className={`truncate text-xs font-bold leading-tight ${
                    pathname.startsWith(GRADER_PATHS.profile) ? "text-white" : "text-muted"
                  }`}
                >
                  Hồ sơ Grader
                </div>
