...
import {
  getWritingTask2CriterionDescriptor,
  WRITING_TASK2_CRITERIA,
  type WritingTask2CriterionScores,
} from "@/lib/writingTask2BandDescriptors";

type Props = {
  scores: WritingTask2CriterionScores;
};

export function WritingTask2CriteriaPanel({ scores }: Props) {
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      {WRITING_TASK2_CRITERIA.map((criterion) => {
        const band = scores[criterion.key];
        const lines = getWritingTask2CriterionDescriptor(band, criterion.key);
