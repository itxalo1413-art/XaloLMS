...
export function EntranceBookingModal({
  ...
  const [grader, setGrader] = useState<string>(initialGrader || MOCK_TEST_TEACHER_OPTIONS[0]);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      />
      <div className="relative w-full max-w-2xl rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-950/60">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-xl bg-amber-500/15 border border-amber-500/30 flex items-center justify-center text-amber-400 font-black">
              🗓️
            </div>
            <div>
              <h2 className="text-base font-black text-white">Đặt Lịch Grader Chấm Entrance Test</h2>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
          {error && (
            <div className="rounded-xl border border-rose-500/30 bg-rose-500/10 px-4 py-3 text-xs font-semibold text-rose-400 flex items-center gap-2">
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-3.5 space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-slate-300">Chọn nhanh từ danh sách Lead BCB</label>
              <span className="text-[10px] text-slate-500 font-medium">Hoặc nhập trực tiếp bên dưới</span>
            </div>
            <select
              value={selectedLeadId}
              onChange={(e) => setSelectedLeadId(e.target.value)}
              className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3.5 py-2 text-xs text-slate-100 outline-none focus:border-amber-500/60 transition-all cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-1.5">
                Tên ứng viên <span className="text-rose-400">*</span>
              </label>
              <input
                type="text"
                required
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-1.5">
                Loại bài Test Entrance <span className="text-rose-400">*</span>
              </label>
              <div className="grid grid-cols-3 gap-1.5 p-1 rounded-xl bg-slate-800 border border-slate-700 text-xs font-bold">
                <button
                  type="button"
            ...
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-1.5">Hình thức thi</label>
              <div className="grid grid-cols-2 gap-1.5 p-1 rounded-xl bg-slate-800 border border-slate-700 text-xs font-bold">
                <button
                  type="button"
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-400 mb-1.5">
                  Nhân viên Grader chấm <span className="text-rose-400">*</span>
                </label>
                <select
                  value={grader}
                  onChange={(e) => {
                  ...
                  className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3.5 py-2.5 text-sm font-bold text-slate-100 outline-none focus:border-amber-500/60 cursor-pointer"
                >
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 mb-1.5">
                  Ngày thi / Hạn chấm <span className="text-rose-400">*</span>
                </label>
                <input
                  type="date"
                  required
                  ...
                  className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3.5 py-2.5 text-sm text-slate-100 outline-none focus:border-amber-500/60 cursor-pointer"
                />
              </div>
            </div>
            ...
            <div>
              ...
              {availableSlotsForDate.length > 0 ? (
                <div className="flex flex-wrap gap-2 max-h-24 overflow-y-auto p-2 rounded-xl bg-slate-900 border border-slate-800">
                ...
                <p className="text-[11px] text-slate-500 italic p-2 rounded-xl bg-slate-900/60 border border-slate-800/80">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
              <div>
                <label className="block text-xs font-bold text-slate-400 mb-1.5">
                  Giờ thi / Giờ bắt đầu <span className="text-rose-400">*</span>
                </label>
                <input
                  type="time"
                  required
                  ...
                  className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3.5 py-2.5 text-sm text-slate-100 outline-none focus:border-amber-500/60"
                />
              </div>
              ...
              {format === "online" && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3.5 py-2.5 text-sm text-slate-100 placeholder-slate-500 outline-none focus:border-amber-500/60"
                  />
                </div>
              )}
            </div>
          ...
          <div className="space-y-3">
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-1.5">Link Đề Thi (nếu có)</label>
              <input
                type="url"
                value={examLink}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-1.5">Ghi chú từ Sale</label>
              <textarea
                rows={2}
                value={note}
                onChange={(e) => setNote(e.target.value)}
                placeholder="VD: Khách cần kết quả trước 17h ngày mai, lưu ý phát âm Part 2..."
                className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3.5 py-2.5 text-sm text-slate-100 placeholder-slate-500 outline-none focus:border-amber-500/60 resize-none"
              />
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-800">
            <button
              type="button"
              onClick={onClose}
              className="rounded-xl border border-slate-700 px-5 py-2.5 text-xs font-bold text-slate-400 hover:bg-slate-800 transition-colors"
            >
              Hủy
            </button>
            <button
              type="submit"
              disabled={submitting}
              className="flex items-center gap-2 rounded-xl bg-amber-500 hover:bg-amber-400 px-6 py-2.5 text-xs font-black text-slate-950 transition-all shadow-md hover:shadow-amber-500/20 disabled:opacity-50"
            >
              {submitting ? (
                <>
